...
void afisareAvion(struct Avion avion) {
	printf("Compania %s are un avion de %d locuri cu pretul de %5.2f pe loc\n", avion.companie, avion.nrPasageri, avion.pretBilet);
}
...
struct Avion* copieazaPrimeleNAvioane(struct Avion* avioane, int nrAvioane, int nrAvioaneCopiate) {
...
struct Avion getPrimulAvionDupaCompanie(struct Avion* avioane, int nrAvioane, const char* companie) {
	...
	return initializareAvion(-1, "NOT APPLICABLE", -1);
}
...
void main() {
	...
	struct Avion* avioaneCopiate = copieazaPrimeleNAvioane(avioane, nrAvioane, nrAvioaneCopiate);
	printf("\n\n");
	...
	printf("\n\nVector avioane cu bilete reduse: \n\n");
